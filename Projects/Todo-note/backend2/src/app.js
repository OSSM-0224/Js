...
import Note1Model from "./models/list1.model.js";
...
app.post("/api/notes", async (req, res) => {
  ...
  const newNote = await Note1Model.create({ title, description });
...
app.get("/api/get-notes", async (req, res) => {
  const getNote = await Note1Model.find();
...
app.patch("/api/update-note/:id", async (req, res) => {
  ...
  const updateNote = await Note1Model.findById(id);
 ...
 * @description Deleting the note using ID
 * @access Public
 **/
app.delete("/api/delete-note/:id", async (req, res) => {
  const { id } = req.params;
  const deleteNote = await Note1Model.findById(id);
